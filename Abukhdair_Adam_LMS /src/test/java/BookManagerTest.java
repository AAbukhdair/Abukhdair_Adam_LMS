...
public class BookManager {
    ...
    public boolean checkoutBookById(String id) {
        for (Book book : books) {
            if (book.getId().equals(id) && !book.isCheckedOut()) {
                book.setCheckedOut(true, LocalDate.now().plusWeeks(4));  // Due in 4 weeks
                return true;
            }
        }
        return false;
    ...
    public boolean checkInBookById(String id) {
        for (Book book : books) {
            if (book.getId().equals(id) && book.isCheckedOut()) {
                book.setCheckedOut(false, null);  // No due date, book is available
                return true;
            }
        }
        return false;
    ...
    public void listBooks() {
        if (books.isEmpty()) {
            ...
            books.forEach(System.out::println);  // Print each book's details
        }
    }
